...
const RealEstateDemoSection: React.FC = () => {
  ...
  const generateAd = async () => {
    ...
    try {
        ...
        if (!apiKey) {
          setErrorMsg("Error de configuración: La API key de Gemini no está configurada. Verifica tu archivo .env.local");
                      ...
                      <div className="flex items-start gap-4 mb-4">
                        ...
                        <div className="flex-grow overflow-hidden">
                            <div className="flex justify-between items-start">
                                ...
                                <button aria-label="Eliminar imagen" onClick={() => handleRemoveImage(img.id)} className="text-gray-500 hover:text-red-400 transition-colors">
             ...
             {showResult && !isGenerating && images.length > 0 && (
                 <div className="animate-fade-in-up flex flex-col h-full">
                     <div className="flex justify-between items-center mb-4">
                        ...
                        {images.length > 1 && (
                            <div className="flex items-center gap-2 bg-black/40 rounded-lg p-1 border border-white/10">
                                <button aria-label="Imagen anterior" onClick={prevResult} disabled={activeResultIndex === 0} className="p-1 hover:bg-white/10 rounded disabled:opacity-30 text-white">
                                    <ChevronLeft className="w-4 h-4" />
                                </button>
                                <span className="text-xs font-mono text-gray-300 px-2 min-w-[50px] text-center">
                                    {activeResultIndex + 1} / {images.length}
                                </span>
                                <button aria-label="Imagen siguiente" onClick={nextResult} disabled={activeResultIndex === images.length - 1} className="p-1 hover:bg-white/10 rounded disabled:opacity-30 text-white">
                     ...
                     <div className="mt-4 pt-4 border-t border-white/10 text-center">
                         <a href="https://wa.me/573024310220?text=Quiero%20implementar%20la%20IA%20inmobiliaria" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-primary font-bold hover:text-white transition-colors text-sm">
